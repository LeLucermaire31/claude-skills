...
3. **选择执行路径**
4. **列出所有人工决策点**
5. **动态发现领域 Skill**（参考下方 Skill 发现规则）

### Skill 发现规则

Phase 0 不依赖固定映射表，而是**运行时动态发现**可用 Skill：

**发现步骤：**

1. 从 system reminder 提取所有可用 skill 的 `name` + `description`
2. 排除流程/基础设施 skill（brainstorming、debugging、caveman*、codex:* 等）
3. 将项目需求关键词（技术栈、项目类型）与剩余 skill 的 `description` 做语义匹配
4. 输出匹配结果，等待用户确认
...
- **人工决策点**:
  ...
  2. Phase 1a 设计文档确认
  3. Phase 1b 任务计划确认
- **发现的领域 Skill**: ui-ux-pro-max（设计阶段，匹配"前端/Todo UI"）+ gstack（终审 QA，匹配"Web 浏览器测试"）
...
Skill 注入由 Phase 0 动态发现驱动：

1. **动态匹配**：Phase 0 从 system reminder 扫描可用 skill，按技术栈/类型关键词自动匹配
2. **"调用" = 实际执行**：必须使用 `Skill` 工具调用，严禁只说"建议用 xxx skill"
3. **注入时机**：Phase 0 确认后，对应执行阶段开始前
4. **优先级**：用户指令 > Skill 指令 > 默认行为
5. **不确定时**：询问用户选择
6. **新装 Skill 自动生效**：安装新 skill → 重启 session → Phase 0 自动发现，无需手动更新映射
...
在 Phase 0 确认时说明"不需要注入 Skill"即可，工作流会仅使用 superpowers 核心流程。你也可以只剔除某个特定 skill（如"不用 gstack"），保留其他匹配结果。

### Q: 新装的 Skill 没有被发现怎么办？

Skill 发现基于 system reminder 中的 skill 列表。确认 skill 已正确安装并启用（检查 `settings.json` 的 `enabledPlugins`），重启 Claude Code 后再试。
